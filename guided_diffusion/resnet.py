...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_channels=3, use_fp16=False):
...
class CustomResNet(ResNet):
    ...
    def __init__(self, block, num_blocks, num_classes=10, num_channels=3, use_fp16=False):
        super(CustomResNet, self).__init__(block, num_blocks, num_classes, num_channels, use_fp16)
        self.dtype = torch.float16 if use_fp16 else torch.float32
...
def get_model(arch, num_classes, use_fp16=False):
    if arch in ["resnet18", "resnet50"]:
        if arch == "resnet18":
            model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, use_fp16=use_fp16)
        if arch == "resnet50":
            print("yes")
            model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, use_fp16=use_fp16)
    elif arch in ["resnet18_mnist", "resnet50_mnist"]:
        if arch == "resnet18_mnist":
            model = ResNet(
                BasicBlock, [2, 2, 2, 2], num_classes=num_classes, num_channels=1, use_fp16=use_fp16
            )
        if arch == "resnet50_mnist":
            model = ResNet(
                Bottleneck, [3, 4, 6, 3], num_classes=num_classes, num_channels=1, use_fp16=use_fp16
            )
    elif arch in ["resnet18_64", "resnet50_64"]:
        if arch == "resnet18_64":
            model = CustomResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, use_fp16=use_fp16)
        if arch == "resnet50_64":
            model = CustomResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, use_fp16=use_fp16)
    else:
        raise ValueError(
